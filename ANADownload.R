...
meta <- read.csv("hidroweb_metadata(3)_Edited(1).csv")
meta <- meta[which(meta$class=="Fluviometric"),] #14656 total Q stations
meta <- meta[which(meta$basin=="RIO AMAZONAS"),] #1356 Q stations in the Amazon
meta <- meta[,c(1:14)]
...
for (i in 1:length(stationCodes)) {
  ...
  colnames(stationData) <- c("day","month","discharge")
  ...
  #write to .csv
  setwd("D:/Research/Projects/Catchment Classification/flow/ANA2")
